...
# load the raw counts
raw_counts <- read.csv("../dataset/raw_dataset.csv", header=FALSE)
...
cell_types <- read.csv("../dataset/cell_types.csv")
...
###################################################################################
# (DE) analysis for cluster 10
cat('(DE) analysis for cluster 10 cell selection vs myeloids:\n')
df_selection <- read.csv("../coexpression/myeloid_c10_selection.csv")
...
###################################################################################
# (DE) analysis of the cells selected from cluster 8: cells from the myeloids vs other cells
cat('(DE) analysis from cluster 8 cell selection: myeloids vs other cells:\n')
df_selection <- read.csv("../coexpression/myeloid_c8_selection_myeloid_vs_others.csv")
positive_cells <- df_selection[df_selection$selection == 1,'X']
negative_cells <- df_selection[df_selection$selection == 0,'X']
markers <- de_testing(positive_cells, negative_cells) # perform DE testing
print(head(markers)) # print the first rows
